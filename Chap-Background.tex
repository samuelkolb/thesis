...
Several concepts concerning clauses will be important in the context of this thesis:
\begin{definition}
...
\begin{example}
Clause~$c_1$ from example~\ref{ex:functor_free} is range-restricted, because the variables occurring in the head ($x$ and~$y$) also occur in the body. On the other hand, the clause \cen{$friends(x, x) \leftarrow true$} is not range-restricted, since the variable~$x$ only occurs in the head.
\end{example}
...
\begin{example}
Consider the clauses \cen{$c_1 = \{friends(x, y), friends(y, z)\}$\\$c_2 = \{friends(x, y), friends(a, b)\}$.} In clause~$c_1$ the two atoms in the body of the clause are linked by variable $y$. Therefore, clause~$c_1$ is connected. The two atoms occurring in the body of $c_2$, however, do not share a common variable and thus $c_2$ is not connected.
\end{example}
...
\begin{definition}
A refinement operator $\rho$ is complete w.r.t. to a language \sym{L} if and only if $\sym{L} = \rho^*(\square)$, where $\square$ is the empty clause.
\end{definition}
